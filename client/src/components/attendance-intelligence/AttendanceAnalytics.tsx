    ...
    <motion.div whileHover={{ y: -5 }} className="premium-card" style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h3 style={{ margin: 0, fontSize: 'var(--font-md, 18px)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm, 8px)' }}>
          <BarChart2 size={20} color="var(--secondary)" /> Attendance Analytics
        </h3>
        
        <div style={{ display: 'flex', background: isDark ? 'rgba(0,0,0,0.2)' : '#f1f5f9', borderRadius: 'var(--radius-md)', padding: '4px' }}>
          {['Week', 'Month', 'Quarter'].map(v => (
            <button 
              key={v}
              onClick={() => setView(v as any)}
              style={{
                padding: '4px 12px', fontSize: 'var(--font-sm, 12px)', fontWeight: 600, borderRadius: '4px', border: 'none', cursor: 'pointer',
      ...
      <div style={{ flex: 1, minHeight: '300px' }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data.length > 0 ? data : [{ name: 'N/A', attendance: 0, remote: 0 }]}>
            ...
            <Tooltip contentStyle={{ background: isDark ? '#1f2937' : '#fff', border: 'none', borderRadius: 'var(--radius-md)', boxShadow: 'var(--card-shadow, 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06))' }} />
            <Area type="monotone" dataKey="attendance" name="Attendance %" stroke="var(--success)" strokeWidth={3} fillOpacity={1} fill="url(#colorAtt)" />
            <Area type="monotone" dataKey="remote" name="Remote %" stroke="var(--secondary)" strokeWidth={3} fillOpacity={1} fill="url(#colorRem)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '16px', paddingTop: '16px', borderTop: isDark ? '1px solid rgba(255,255,255,0.05)' : '1px solid rgba(0,0,0,0.05)' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 'var(--font-sm, 12px)', color: 'var(--text-muted)' }}>Avg Trust</div>
          <div style={{ fontSize: 'var(--font-md, 18px)', fontWeight: 700, color: 'var(--success)' }}>{stats.avgTrust}%</div>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 'var(--font-sm, 12px)', color: 'var(--text-muted)' }}>Avg Late</div>
          <div style={{ fontSize: 'var(--font-md, 18px)', fontWeight: 700, color: 'var(--danger)' }}>{stats.avgLate}%</div>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 'var(--font-sm, 12px)', color: 'var(--text-muted)' }}>Overtime</div>
          <div style={{ fontSize: 'var(--font-md, 18px)', fontWeight: 700, color: 'var(--primary)' }}>{stats.overtime} hrs</div>
        </div>
      </div>
    </motion.div>
  );
